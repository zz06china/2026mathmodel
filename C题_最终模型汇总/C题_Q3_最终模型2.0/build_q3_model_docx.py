...
TABLE_CAPTIONS = {
    ...
    "8.1 更新时点全组合消融": "表3  更新时点全组合消融",
    "8.2.1 调整阶段分位数": "表4  调整阶段分位数敏感性",
    "8.2.2 统一分位数": "表5  统一分位数敏感性",
    "8.2.3 历史窗口长度": "表6  历史误差窗口敏感性",
    "8.2.4 终端 SOC": "表7  终端SOC设置敏感性",
    "9.1 0:00 计划购电量": "表8  指定日期的0:00计划购电结果",
    "9.2 最终调整购电量": "表9  指定日期的最终调整购电结果",
    "9.3 储能运行结果": "表10  指定日期的储能运行结果",
    "9.4 紧急购电结果": "表11  指定日期的紧急购电结果",
}
...
def build_document():
    ...
    while index < len(lines):
        ...
        if line.startswith("#### "):
            ...
            paper.add_heading(document, current_heading, 3)
            index += 1
            continue
        if line.startswith("### "):
