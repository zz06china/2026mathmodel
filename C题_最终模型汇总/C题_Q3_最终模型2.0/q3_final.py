...
FINAL_TERMINAL_MODE = "band"
FINAL_TERMINAL_BAND = 600.0
TOL = 1e-6
EXPORT_TOL = 5e-5
...
def build_net_load_margins(
    ...
    """Estimate causal stage-specific quantiles of complete net-load errors."""
    if np.isscalar(risk_quantile):
        release_quantiles = np.full(len(RELEASE_SLOTS), float(risk_quantile))
    else:
        release_quantiles = np.asarray(risk_quantile, dtype=float)
    ...
    day_count = len(loads)
    errors = np.full((day_count, len(RELEASE_SLOTS), N), np.nan)
    for day in range(day_count):
        for release_index, release_slot in enumerate(RELEASE_SLOTS):
            load_horizon = load_forecast_horizon(loads, day, release_slot, prior_load)
            if release_slot == 0:
                actual_net = loads[day] - pvs[day]
            elif day + 1 < day_count:
                actual_net = np.r_[
                    (loads[day] - pvs[day])[release_slot:],
                    (loads[day + 1] - pvs[day + 1])[:release_slot],
                ]
            else:
                actual_net = (loads[day] - pvs[day])[release_slot:]
            available = min(N, len(actual_net))
            forecast_net = load_horizon[:available] - pv_forecasts[day, release_index, :available]
            errors[day, release_index, :available] = actual_net[:available] - forecast_net
    ...
    for day in range(day_count):
        ...
        with np.errstate(all="ignore"):
            for release_index, quantile in enumerate(release_quantiles):
                ...
                margins[day, release_index] = np.nan_to_num(margin, nan=0.0)
    return margins
...
def simulate_strategy(
    ...
    robust,
    limit_days=None,
    terminal_mode=FINAL_TERMINAL_MODE,
    terminal_band=FINAL_TERMINAL_BAND,
    terminal_penalty=None,
    verbose=True,
):
...
def summarize(results):
    ...
    daily_emergency = np.asarray([float(np.sum(r["emergency"])) for r in report])
    daily_emergency_cost = np.asarray([float(r["emergency_cost"]) for r in report])
    emergency_slots = np.concatenate([r["emergency"] for r in report]) if report else np.array([])
    ...
    return {
        ...
        "emergency_duration_hours": (
            float(np.sum(emergency_slots > EXPORT_TOL) * DT) if len(emergency_slots) else 0.0
        ),
...
def main():
    ...
    pv_forecasts, _ = load_pv_forecasts(dates, pvs)
    net_load_margins = build_net_load_margins(
        loads,
        pvs,
        pv_forecasts,
        prior_load,
