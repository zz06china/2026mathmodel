...
def optimization_and_response():
    models = ("问题4-2", "问题4-3")
    adaptation_saving = np.array((3.9905, 2.4047))
    perfect_information_gap = np.array((11.5257, 11.6653))
    charge_price = np.array((0.5136, 0.5285))
    discharge_price = np.array((1.1302, 1.1494))

    fig, axes = plt.subplots(1, 2, figsize=(9.2, 3.8))
    x = np.arange(len(models))
    width = 0.32
    axes[0].bar(x - width / 2, adaptation_saving, width, color=BLUE, label="因果调度节省")
    axes[0].bar(x + width / 2, perfect_information_gap, width, color=GRAY, label="完全信息差距")
