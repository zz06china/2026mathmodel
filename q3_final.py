...
def load_pv_forecasts(dates, actual_pv):
    ...
    for day in range(len(dates)):
        start = max(0, day - ERROR_WINDOW)
        if day == start:
            continue
        history = errors[start:day]
        with np.errstate(all="ignore"):
            margin = np.nanquantile(history, RISK_QUANTILE, axis=0)
        margins[day] = np.maximum(np.nan_to_num(margin, nan=0.0), 0.0)
...
def solve_schedule(load_forecast, pv_forecast, horizon_prices, e_start, base_plan=None):
    ...
    model += pulp.lpSum(cost_terms) + 1e-8 * pulp.lpSum(
        charge[k] + discharge[k] + spill[k] for k in range(N)
    )
...
def simulate_strategy(
    ...
    for day in range(total_days):
        ...
        for release_position, release_slot in enumerate(releases):
            ...
            schedule = solve_schedule(load_horizon, pv_horizon, horizon_prices, current, base)
            if release_slot == 0:
        ...
        e_start = current
        if (day + 1) % 30 == 0 or day + 1 == total_days:
